...
//        }
		...
		FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("providerEmail",
				provider.getEmail());
	...
	public String verifyOtp() throws Exception {
		...
		if (latestOtp == null) {
			...
			FacesContext.getCurrentInstance().addMessage("otp", new FacesMessage(FacesMessage.SEVERITY_ERROR,
	...
	public String updatePassword() throws Exception {
		...
		System.out
				.println("Inputs → email: " + email + " | newPwd length: " + (newPwd != null ? newPwd.length() : "null")
		...
		if (email == null || email.trim().isEmpty() || newPwd == null || newPwd.trim().isEmpty() || confirmPwd == null
		...
		if (!lengthOK || !upperOK || !digitOK || !specialOK) {
			System.out.println("❌ Validation failed: weak password" + " | lengthOK=" + lengthOK + " | upperOK="
					+ upperOK + " | digitOK=" + digitOK + " | specialOK=" + specialOK);
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"Password must be ≥8 chars, include uppercase, number & special char.", null));
			return null;
		}
	...
	public String login() {
		...
		// 2. Normalize inputs
		String email = (provider.getEmail() == null) ? "" : provider.getEmail().trim().toLowerCase();
		String plainPassword = (provider.getPassword() == null) ? "" : provider.getPassword().trim();
		System.out.println("Normalized → email: '" + email + "' | password present? " + !plainPassword.isEmpty());
		...
		try {
			...
			e.printStackTrace();
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_FATAL, "System error. Please try again later.", null));
			return null;
		}
		...
		System.out.println("✔ authentication successful for: " + dbProvider.getProviderName());
		...
		if (status == LoginStatus.APPROVED) {
			// store in session
			FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("loggedInProvider", dbProvider);
			...
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Your account is locked or inactive.", null));
			FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("providerId",
...
//        }
	...
	public String verifyLoginOtp() throws Exception {
		...
		String sessionEmail = (String) FacesContext.getCurrentInstance().getExternalContext().getSessionMap()
				.get("providerEmail");
	...
	ProviderDao dao = providerDaoImpl;
	...
	public String resetProviderPassword() {
		FacesContext context = FacesContext.getCurrentInstance();
		boolean isValid = true;
		String success = null;
		// 1.Step1 :check validation before calling daoImpl

		// if old password null
		if (oldPassword == null || oldPassword.trim().isEmpty()) {
			context.addMessage("changePwdForm:oldPwd", new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"Old Password Required", ConstMessage.OLD_PASSWORD_REQUIRED.getMessage()));
			isValid = false;
		}
		// if new Password null
		if (newPassword == null || newPassword.trim().isEmpty()) {
			context.addMessage("changePwdForm:newPwd", new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"New Password Required", ConstMessage.NEW_PASSWORD_REQUIRED.getMessage()));
			isValid = false;
		}
		// if confirm password null
		if (confirmPassword == null || confirmPassword.trim().isEmpty()) {
			context.addMessage("changePwdForm:confirmPwd", new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"Confirm Password Required", ConstMessage.CONFIRM_PASSWORD_REQUIRED.getMessage()));
			isValid = false;
		}
		...
		if (newPassword != null && confirmPassword != null && !newPassword.equals(confirmPassword)) {
			...
			context.addMessage("changePwdForm:confirmPwd", new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"Password Mismatch", ConstMessage.PASSWORD_MISMATCH.getMessage()));
			isValid = false;
		}

		if (!isValid) {
			return null;// same page if validation fails
		}

		try {
			// the controller fetches the current user from the session

			Provider currentProvider = (Provider) context.getExternalContext().getSessionMap().get("loggedInProvider");

			// Step 2 : Database call (doaImpl)
			System.err.println("the provider we are resetting the password : " + currentProvider);
			success = dao.resetPassword(currentProvider.getProviderId(), oldPassword, newPassword);

		} catch (Exception e) {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"An error occurred while changing the password.", null));
			System.out.println("An error occurred while changing the password.NULL");
			return null;
		}
		if (success == null) {
			return null;
